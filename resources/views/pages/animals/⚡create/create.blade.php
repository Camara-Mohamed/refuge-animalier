<div class="flex flex-col gap-4">
    ...
    <h2 class="font-serif font-bold text-2xl text-blue-strong">{{ __('admin/animals.create_title') }}</h2>
    ...
    <form wire:submit="save" class="flex flex-col gap-6 max-w-2xl p-6 md:p-8 bg-white rounded-lg shadow-[0px_5px_20px_0px_rgba(0,0,0,0.10)] border border-red-strong/20">
        <x-forms.fieldset title="{{ __('admin/animals.general_info') }}">
            ...
            <div class="flex flex-col gap-2">
                <label for="gender" class="font-medium font-serif text-blue-strong">
                    {{ __('admin/animals.gender') }}
                    <small><abbr class="text-red-normal" title="{{ __('admin/animals.required_field') }}">*</abbr></small>
                </label>
                <x-forms.select for="gender" wire:model="gender" :required="true" class="w-full">
                    <option value="">{{ __('admin/animals.choose') }}</option>
                    <option value="male">{{ __('admin/animals.male') }}</option>
                    <option value="female">{{ __('admin/animals.female') }}</option>
                </x-forms.select>
        ...
        <x-forms.fieldset title="{{ __('admin/animals.photo') }}">
            <div class="flex flex-col gap-2">
                <label for="avatarFile" class="font-medium font-serif text-blue-strong">{{ __('admin/animals.main_photo') }}</label>
